    ...
    public class BufferSchema {
        ...
        public Action<BufferPlus, dynamic> BuildWriteAction(Definition definition) {
            ...
            if (definition.Type == TypeString.Array) {
                ...
                /*
                if (property.Type == TypeString.Bool) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data));
                }

                if (property.Type == TypeString.Int8) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data));
                }

                if (property.Type == TypeString.Int16BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.Int16LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.Int32BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.Int32LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.Int64BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.Int64LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.UInt8) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data));
                }

                if (property.Type == TypeString.UInt16BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.UInt16LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.UInt32BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.UInt32LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.UInt64BE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }

                if (property.Type == TypeString.UInt64LE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.FloatBE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }
                ...
                if (property.Type == TypeString.DoubleLE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), true);
                }

                if (property.Type == TypeString.DoubleBE) {
                    action = (bp, data) => bp.WriteArray(property.GetValue(data), false);
                }
                */
        ...
        public Definition ParseDefinition(JToken json,string name = null) {
        ...
        public class TypeString {
            ...
            public const string FloatBE = "floatbe";
            public const string FloatLE = "floatle";
            public const string Float32BE = "floatbe";
            public const string Float32LE = "floatle";
            public const string DoubleBE = "doublebe";
            public const string DoubleLE = "doublele";
            public const string Float64BE = "doublebe";
            public const string Float64LE = "doublele";
